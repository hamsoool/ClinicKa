...
export default function StaffLayout() {
  ...
  const menuItems = [
    { path: '/staff', label: 'Dashboard', icon: Home },
    { path: '/staff/submissions', label: 'Student Submissions', icon: FileText },
    { path: '/staff/records', label: 'Medical Records', icon: FolderOpen },
    { path: '/staff/reports', label: 'Reports / Analytics', icon: BarChart3 },
    { path: '/staff/certificates', label: 'Certificates', icon: Award },
  ...
  const topNavItems = [
    ...
    { path: '/staff/submissions', label: 'Status' },
    { path: '/staff/records', label: 'Records' },
    { path: '/staff/certificates', label: 'Certificates' },
  ];
  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      <button
        ...
        aria-label={sidebarOpen ? 'Close staff navigation menu' : 'Open staff navigation menu'}
      >
          ...
          <div className="p-6 border-b border-sidebar-border">
            <div className="flex items-center gap-3 mb-4">
              <div className="relative group">
                ...
                <label htmlFor="staff-profile-upload" className="absolute inset-0 bg-black/50 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity">
              ...
              <div>
                ...
                <p className="text-sm opacity-90">
                  {me?.staff?.position || me?.profile.email || 'Health Services'}
                </p>
              </div>
            </div>
            <Button
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 border-t bg-background/95 backdrop-blur">
        <div className="grid grid-cols-3 px-2 py-2 pb-[env(safe-area-inset-bottom)]">
          {[
            { path: '/staff', label: 'Home', icon: Home },
            { path: '/staff/records', label: 'Records', icon: FolderOpen },
            { path: '/staff/certificates', label: 'Certificates', icon: Award },
          ].map((item) => {
            const Icon = item.icon;
            const active = isActive(item.path);

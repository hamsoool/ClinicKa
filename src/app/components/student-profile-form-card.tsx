...
function formatIsoToMdY(isoString: string): string {
  const cleanIso = String(isoString || '').split('T')[0].trim();
  const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(cleanIso);
...
export function StudentProfileFormCard({
  ...
  const personalGridClass = isPlain
    ? 'grid w-full min-w-0 gap-5 sm:grid-cols-2 lg:grid-cols-3'
    : 'grid w-full min-w-0 gap-5 md:grid-cols-2';
  const contactGridClass = isPlain
    ? 'grid w-full min-w-0 gap-5 sm:grid-cols-2 lg:grid-cols-3'
    : 'grid w-full min-w-0 gap-5 md:grid-cols-2';
  const addressColClass = isPlain ? 'w-full min-w-0 sm:col-span-1 lg:col-span-2' : 'w-full min-w-0';
  const academicGridClass = isPlain
    ? 'grid w-full min-w-0 gap-5 sm:grid-cols-2 lg:grid-cols-4'
    : 'grid w-full min-w-0 gap-5 md:grid-cols-2';
    ...
    <>
      ...
      {(category === 'all' || category === 'academic') && (
        <div className="space-y-4">
          ...
          <div className={academicGridClass}>
            <div className="w-full min-w-0">
  ...
  if (isPlain) {
    return <div className="w-full min-w-0 space-y-8">{categoriesContent}</div>;
  }
  ...
  return (
    <Card className="box-border w-full min-w-0 overflow-hidden rounded-[18px] border border-outline-variant/30 bg-surface-container-lowest !gap-0">
      ...
      <CardContent className="space-y-6 px-4 pt-4 sm:px-6">
        {categoriesContent}
      </CardContent>
    </Card>
  );
